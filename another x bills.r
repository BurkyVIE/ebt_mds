...
Cnt <- he %>% filter(D_Cnt < 0) %>% pull(Date) %>% c(lubridate::ymd("2004-8-4"), .) %>% as.numeric() %>% diff() %>% tibble(DDiff = ., Cat = "Cnt")
Val <- he %>% filter(D_Val < 0) %>% pull(Date) %>% c(lubridate::ymd("2004-8-4"), .) %>% as.numeric() %>% diff() %>% tibble(DDiff = ., Cat = "Val")
Hts <- he %>% filter(D_Hts < 0) %>% pull(Date) %>% c(lubridate::ymd("2004-8-4"), .) %>% as.numeric() %>% diff() %>% tibble(DDiff = ., Cat = "Hts")
