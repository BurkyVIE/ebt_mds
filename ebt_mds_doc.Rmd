...
colors <- viridis::viridis(7, begin = .1, end = .9)
...
```{r year1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.asp=0.5625}
...
p <- ggplot(data = tmp, mapping = aes(x = Year, y = Count, group = Denomination)) +
  ...
  scale_fill_viridis_d(direction = -1, begin = .1, end = .9) +
  theme_bw()
...
Plot of last 13 streaks:

```{r hits2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.asp=0.5625}

p <- streaks %>% arrange(From) %>% tail(13) %>%
  ggplot() +
  geom_rect(mapping = aes(xmin = From - .5, xmax = To + .5, ymin = 0, ymax = Hits/Days, fill = HitRt)) +
  scale_x_date(date_labels = "%m/%Y") +
  scale_fill_viridis_c(name = "Hit ratio", direction = -1, begin = .1, end = .9) +
  labs(x = "Time", y = "Hits per Day") +
  theme_bw()
