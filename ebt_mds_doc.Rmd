...
**`r stat$Count %>% format(big.mark = ",")` bills** (`r (stat$Count / days$total) %>% sprintf(., fmt = '%#.2f')` /day) were entered with a total value of **`r stat$Value %>% format(big.mark = ",")` Euro** (mean = `r (stat$Value / stat$Count) %>% sprintf(., fmt = '%#.2f')`); **`r stat$Hits %>% format(big.mark = ",")` hits** (= `r (stat$Hits / days$total) %>% sprintf(., fmt = '%#.2f')` /day) occurred (hit-ratio = 1 : `r (stat$Count / stat$Hits) %>% sprintf(., fmt = '%#.2f')`).\
...
Active contribution to EBT was recorded in `r (substr(ebt_mds_full$Date, 1, 7) |> unique() |> length())` different months.\
The seven month with most entered bills were:
...
ebt_mds_full %>% 
  ...
  # for later plots
  dat <- streaks %>% arrange(From) %>% tail(15)
