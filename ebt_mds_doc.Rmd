...
subtitle: "https://www.eurobilltracker.com"
author: "Burky"
output:
...
ebt_mds_full <-
  ...
  full_seq(1) %>%                     # einen Vektor aller mÃ¶glichen Zeitpunkte erzeugen ...
  tibble(Date = .) %>%                # in einen Tibble umwandeln ...
  left_join(ebt_mds, by = "Date") %>% # und die Daten zu den Eingabezeitpunkten einfÃ¼gen.
...
# Immer englische AbkÃ¼rzungen fÃ¼r die Wochentage verwenden [wday() nimmt aus locale()]
...
stat <- ebt_mds_full %>% summarise_at(.vars = vars(Count, Value, Hits), .f = ~ sum(., na.rm = TRUE), .groups = "drop") %>%
  bind_cols(
    ebt_mds_full %>% summarise_at(.vars = "Loc", .f = ~ unlist(.) %>% unique() %>% length(), .groups = "drop")
  )
```
...
p <- ebt_mds_full %>%
  ...
  geom_abline(slope = c(100, 200), color = "grey67", linetype = "dashed", size = 1) +
...
ebt_mds_full %>% 
  ...
  summarise(across(.cols = `€ 5`:`€ 500`, .fns = sum), .groups = "drop") %>% 
