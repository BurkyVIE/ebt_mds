...
ebt_mds_full <-
  ...
  mutate(Day = lubridate::wday(x = Date, week_start = 1, label = TRUE),
         Deno = map(.x = Deno, .f = ~ if(is.null(.)) integer(7) else as.integer(.)), # ersetze die aus dem Joinen (auch schon bei der Erstellung) entstandenen NULL
...
stat <- ebt_mds_full %>% summarise_at(.vars = vars(Count, Value, Hits), .funs = ~sum(., na.rm = TRUE)) %>%
  bind_cols(
    ebt_mds_full %>% summarise_at(.vars = "Loc", .funs = ~ unlist(.) %>% unique() %>% length())
  )
```
...
The **five longest periods** cover **`r sum(tmp$Days) %>% format(big.mark = ",")` days** (= `r (sum(tmp$Days) / days$total * 100) %>% sprintf(., fmt = '%#.2f')`%), **`r sum(tmp$Count) %>% format(big.mark = ",")` bills** (= `r (sum(tmp$Count) / stat$Count * 100) %>% sprintf(., fmt = '%#.2f')`%), a total value of **`r sum(tmp$Value) %>% format(big.mark = ",")` Euro** (= `r (sum(tmp$Value) / stat$Value * 100) %>% sprintf(., fmt = '%#.2f')`%) and **`r sum(tmp$Hits) %>% format(big.mark = ",")` hits** (= `r (sum(tmp$Hits) / stat$Hits * 100) %>% sprintf(., fmt = '%#.2f')`%).
...
ebt_mds_full$Deno %>%
  ...
  mutate(Val = substr(Denomination, 2, 4) %>% as.integer(),
         Denomination = factor(Denomination, labels = c(5, 10, 20, 50, 100, 200, 500) %>%
                                 ...
                                 paste0("€ ", .)),
         Value = Count * Val,
         Pct_C = (Count / sum(Count) * 100) %>% round(., 2),
         Pct_V = (Value / sum(Value) * 100) %>% round(., 2)) %>% 
  select(Denomination, Count, Pct_C, Value, Pct_V) -> tmp
  knitr::kable(tmp, format.args = list(big.mark = ','))

```

###Yearly Variation
Proportion of denominations by Year:
...
ebt_mds_full %>% 
  ...
  summarise_at(.vars = vars(`€ 5`:`€ 500`), .funs = ~ sum(.)) %>% 
...
###Monthly Variation
```{r monthly, echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.asp=0.5625}

ebt_grpd <- ebt_mds_full %>%
  # Zusammenfassen der Daten pro Monat
  group_by(Period = floor_date(Date, unit = "month")) %>% 
  summarise(Days = n(),
...
p <- ggplot(data = p_dat, mapping = aes(x = Period, y = Count)) + 
  geom_point(mapping = aes(color = Color), shape = 4, size = 0.8, alpha = 0.65, show.legend = FALSE) +
  geom_smooth(color = colors[4], method = "loess", span = 1.5 / log(ebt_grpd$Period %>% length()),  se = FALSE) +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
  scale_color_manual(values = c("old" = colors[2], "recent" = colors[6])) +
  labs(x = "Time", y = "") +
...
plot(p)
```
```{r monthly2, echo=FALSE}

p_dat <- p_dat %>%
  filter(What == "Notes per Day") %>%
  arrange(Count) %>% 
  transmute(Time = format(Period, "%Y-%m"), Count = Count)
```
The month with **least** entered **bills** (= `r p_dat$Count %>% head(1) %>% sprintf(., fmt = '%#.2f')` /day) was **`r p_dat$Time %>% head(1)`**, the month with **most** entered **bills** (= `r p_dat$Count %>% tail(1) %>% sprintf(., fmt = '%#.2f')` /day) was **`r p_dat$Time %>% tail(1)`**.  
For the `r p_dat %>% nrow() %>% format(big.mark = ",")` recorded month the **median** was at **`r p_dat$Count %>% median() %>% sprintf(., fmt = '%#.2f')` bills/day**.
