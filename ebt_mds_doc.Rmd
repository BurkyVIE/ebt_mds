...
dat0 <- semi_join(tmp, dat) %>% mutate(Mid = as_date(From  + as.duration(interval(From, To)) / 2))

p <- ggplot() +
  geom_rect(data = dat, mapping = aes(xmin = From - .5, xmax = To + .5, ymin = 0, ymax = Hits/Days, fill = HitRt)) +
  geom_point(data = dat0, mapping = aes(x = Mid, y = Hits/Days), color = "red", size = 2, shape = 17) +
