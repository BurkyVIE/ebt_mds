...
p <- ggplot() +
  ...
  scale_x_continuous(expand = expansion(add = .01), labels = (\(x) paste0(x, "°", ifelse(x < 0, "W", "E")))) + # solves problems with incorrect interpretation of 'degrees'
  scale_y_continuous(expand = expansion(add = .01), labels = (\(y) paste0(y, "°", ifelse(y < 0, "S", "N")))) +
